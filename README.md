...
* Change the dimensions in the init function. 
    ...
    * *Answer: It moves the window around to the specified starting point (changing in reference to pixels)*
...
* Change the color of the quad by modifying the call to `glColor3f`.
    * What is the range of values for each part of the color?
      * The range is 0 to 1 (must be a fraction)
...
* Swap the third and fourth coordinates of the quad.
    * What does it look like?
      * It kind of looks like it collapsed on itself. It looks like two triangles overlapping.
...
* Draw a new shape using GL_TRIANGLE_STRIP.
    * If you add a vertex n to a triangle strip (where n >= 3), which two vertices does it connect to?
      * The two on the right of the triangle 
...
*You would use GL_TRIANGLE_FAN because you need to be able to have a center vertex and have it "fan" around it?
